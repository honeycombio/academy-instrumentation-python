...
In the project root directory, update the `.env` file with the following:
...
1. Make sure you are in the `01_autoinstrument` directory.
...
5. Open the Dockerfile. Modify the command to run the app in the Dockerfile:

```Dockerfile
CMD ["opentelemetry-instrument", "flask", "run", "-p 10115", "--host=0.0.0.0"]
```
