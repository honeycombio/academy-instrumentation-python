...
class DiskUsageMonitor:
    ...
    def get_free_space_and_stats(self):
        return {"value": self._value, "as_of": self._last_check_time,
                "check_count": self._how_many_times_have_we_checked}
...
class CustomSpanProcessor(SpanProcessor):
    ...
    def on_start(self,
                 ...
                 parent_context=None):
        span.set_attribute(
            "app.custom_span_processor.tmp_free_space",
            self._disk_usage_monitor.get_free_space())
        span.set_attribute(
            "app.custom_span_processor.tmp_free_space_stats", json.dumps(
                self._disk_usage_monitor.get_free_space_and_stats()))
